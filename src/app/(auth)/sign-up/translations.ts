const translations = {
  en: {
    createAccount: "Create a new account",
    description: "It's not difficult, you just need to enter some data and you're done!",
    fullName: "Full Name",
    firstName: "First Name",
    lastName: "Last Name",
    role: "Role",
    selectRole: "Select Role",
    roleOrganization: "Role Organization",
    selectRoleOrganization: "Select Role Organization",
    email: "Email",
    emailPlaceholder: "Write Your Email",
    password: "Password",
    passwordPlaceholder: "Write Your Password",
    phone: "Phone",
    phonePlaceholder: "Enter Your Phone",
    gender: "Gender",
    selectGender: "Select Gender",
    nationality: "Nationality",
    nationalityPlaceholder: "Enter Your Nationality",
    academicAccreditation: "Academic accreditation",
    browseOrDrop: "Browse or Drop",
    establishmentDecision: "Establishment decision",
    agreeTerms: "By registering, you agree to the Terms of Service and Privacy Policy.",
    signUp: "Sign Up",
  },
  ar: {
    createAccount: "إنشاء حساب جديد",
    description: "ليس الأمر صعبًا، فقط أدخل بعض البيانات وانتهيت!",
    fullName: "الاسم الكامل",
    firstName: "الاسم الأول",
    lastName: "اسم العائلة",
    role: "الدور",
    selectRole: "اختر الدور",
    roleOrganization: "منظمة الدور",
    selectRoleOrganization: "اختر منظمة الدور",
    email: "البريد الإلكتروني",
    emailPlaceholder: "اكتب بريدك الإلكتروني",
    password: "كلمة المرور",
    passwordPlaceholder: "اكتب كلمة المرور الخاصة بك",
    phone: "رقم الهاتف",
    phonePlaceholder: "أدخل رقم هاتفك",
    gender: "الجنس",
    selectGender: "اختر الجنس",
    nationality: "الجنسية",
    nationalityPlaceholder: "أدخل جنسيتك",
    academicAccreditation: "الاعتماد الأكاديمي",
    browseOrDrop: "استعرض أو أسقط",
    establishmentDecision: "قرار التأسيس",
    agreeTerms: "عند التسجيل، فإنك توافق على شروط الخدمة وسياسة الخصوصية.",
    signUp: "إنشاء حساب",
  },
  fr: {
    createAccount: "Créer un nouveau compte",
    description: "Ce n'est pas difficile, il vous suffit d'entrer quelques données et c'est tout!",
    fullName: "Nom complet",
    firstName: "Prénom",
    lastName: "Nom de famille",
    role: "Rôle",
    selectRole: "Sélectionnez le rôle",
    roleOrganization: "Organisation du rôle",
    selectRoleOrganization: "Sélectionnez l'organisation du rôle",
    email: "E-mail",
    emailPlaceholder: "Entrez votre e-mail",
    password: "Mot de passe",
    passwordPlaceholder: "Entrez votre mot de passe",
    phone: "Téléphone",
    phonePlaceholder: "Entrez votre téléphone",
    gender: "Genre",
    selectGender: "Sélectionnez le genre",
    nationality: "Nationalité",
    nationalityPlaceholder: "Entrez votre nationalité",
    academicAccreditation: "Accréditation académique",
    browseOrDrop: "Parcourir ou déposer",
    establishmentDecision: "Décision d'établissement",
    agreeTerms: "En vous inscrivant, vous acceptez les Conditions de service et la Politique de confidentialité.",
    signUp: "S'inscrire",
  },
  ru: {
    createAccount: "Создать новый аккаунт",
    description: "Это несложно, просто введите данные, и вы готовы!",
    fullName: "Полное имя",
    firstName: "Имя",
    lastName: "Фамилия",
    role: "Роль",
    selectRole: "Выберите роль",
    roleOrganization: "Организация роли",
    selectRoleOrganization: "Выберите организацию роли",
    email: "Электронная почта",
    emailPlaceholder: "Введите вашу электронную почту",
    password: "Пароль",
    passwordPlaceholder: "Введите ваш пароль",
    phone: "Телефон",
    phonePlaceholder: "Введите ваш телефон",
    gender: "Пол",
    selectGender: "Выберите пол",
    nationality: "Национальность",
    nationalityPlaceholder: "Введите вашу национальность",
    academicAccreditation: "Академическая аккредитация",
    browseOrDrop: "Просмотреть или перетащить",
    establishmentDecision: "Решение об учреждении",
    agreeTerms: "Регистрируясь, вы соглашаетесь с Условиями использования и Политикой конфиденциальности.",
    signUp: "Зарегистрироваться",
  },
};

export default translations;
